...
export default function ClubMemberRolesAdmin({ embedded = false }) {
  const { user } = useAuth();
  ...
  const loadRoles = async () => {
    setLoading(true);
    try {
      const response = await clubMemberRolesApi.getAll(showInactive);
      if (response.success) {
        setRoles(response.data || []);
      }
    } catch (err) {
      console.error('Error loading club member roles:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingRole) {
        const response = await clubMemberRolesApi.update(editingRole.id, formData);
        if (response.success) {
        ...
        const response = await clubMemberRolesApi.create(formData);
        if (response.success) {
  ...
  const handleDelete = async (role) => {
    if (role.isSystemRole) {
      alert('Cannot delete system roles (Admin, Member)');
      return;
    }
    if (!confirm(`Are you sure you want to delete "${role.name}"?`)) return;

    try {
      const response = await clubMemberRolesApi.delete(role.id);
      if (response.success) {
        if (showInactive) {
          setRoles(roles.map(r =>
  ...
  const handleRestore = async (role) => {
    try {
      const response = await clubMemberRolesApi.restore(role.id);
      if (response.success) {
        setRoles(roles.map(r =>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm p-12 text-center">
            ...
            <p className="text-gray-500 mb-6">Get started by adding your first club member role.</p>
            <button
                  ...
                  <tr key={role.id} className={!role.isActive ? 'bg-gray-50 opacity-60' : ''}>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1">
                        ...
                        {!role.canManageClub && !role.canManageMembers && !role.canPostAnnouncements && (
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={formData.canManageClub}
                        onChange={(e) => setFormData({ ...formData, canManageClub: e.target.checked })}
                        className="w-4 h-4 text-green-600 border-gray-300 rounded focus:ring-green-500"
                      />
                      <UserCog className="w-4 h-4 text-purple-600" />
                      <span className="text-sm text-gray-700">Can manage club settings</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                      ...
                      <span className="text-sm text-gray-700">Can manage members (approve/remove)</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={formData.canPostAnnouncements}
                        onChange={(e) => setFormData({ ...formData, canPostAnnouncements: e.target.checked })}
                        className="w-4 h-4 text-green-600 border-gray-300 rounded focus:ring-green-500"
                      />
                      <Megaphone className="w-4 h-4 text-orange-600" />
                      <span className="text-sm text-gray-700">Can post announcements</span>
                    </label>
                  </div>
                </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Club Member Roles</h1>
                <p className="text-sm text-gray-500">Define roles and permissions for club members</p>
              </div>
            </div>
            <button
              onClick={handleCreate}
